...
export const TVShows: FC = () => {
  ...
  const { showsQuery, showsPage, setShowsQueryValue, setShowsPageValue } =
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (showsQuery.length < 3) {
        getTopRatedMovies();
      } else {
        searchMovies(showsQuery, showsPage);
      }
    }, 1000);
    return () => clearTimeout(timeoutId);
    // eslint-disable-next-line
  }, [showsQuery, showsPage]);
  ...
  const props = {
    query: showsQuery,
    setQuery: setShowsQueryValue,
    currentPage: showsPage,
    setCurrentPage: setShowsPageValue,
    data: movies,
    dataLength,
    handleSearch,
    hasPaggination,
